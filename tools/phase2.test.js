...
function testCanonicalModuleSurface() {
    ...
    assert.ok(advisorSource.includes('_secureActionButtonPool'), 'Secure buttons must have a bounded reuse pool.');
    assert.ok(advisorSource.includes('AcquireSecureActionButton'), 'Secure buttons must be acquired through the reuse helper.');
    assert.ok(advisorSource.includes('pool[poolKey]'), 'Secure buttons must be returned to per-config pool buckets.');
    assert.ok(!advisorSource.includes('CreateFrame("Button", nil, f, "SecureActionButtonTemplate")'),
        'DisplayResults must not allocate protected buttons directly per render.');
